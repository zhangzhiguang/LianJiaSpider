...
def xiaoqu_spider(db_xq,url_page=u"http://bj.lianjia.com/xiaoqu/pg1rs%E6%98%8C%E5%B9%B3/"):
    ...
    for xq in xiaoqu_list:
        ...
        info_dict.update({u'大区域': xq.find('a', {'class': 'district'}).text.strip()})
        info_dict.update({u'小区域': xq.find('a', {'class': 'bizcircle'}).text.strip()})
        a = xq.find('div', {'class': 'positionInfo'}).text.strip()
        stringarray=a.split('/')
        info_dict.update({u'小区户型': stringarray[1].strip()})
        info_dict.update({u'建造时间': stringarray[2].strip()})
...
def do_xiaoqu_spider(db_xq, region=u"昌平"):
    ...
    a = div[0].get('page-data')
    tmp = html.unescape(a).replace(r'\'', '')
    data = json.loads(tmp)
...
def chengjiao_spider(db_cj,url_page=u"http://bj.lianjia.com/chengjiao/pg1rs%E5%86%A0%E5%BA%AD%E5%9B%AD"):
    ...
    for cj in cj_list:
        ...
        if content:
            info_dict.update({u'小区名称':str(content[0])})
            info_dict.update({u'户型':str(content[1])})
            info_dict.update({u'面积':str(content[2])})
        content=cj.find('div',{'class':'houseInfo'}).text.strip()
        content=content.split('|')
        if content:
            info_dict.update({u'朝向':content[0].strip()})
            flood=cj.find('div',{'class':'positionInfo'}).text.strip()
            info_dict.update({u'楼层': flood.split(' ')[0]})
            info_dict.update({u'建造时间': flood.split(' ')[1]})

        info_dict.update({u'签约时间':cj.find('div',{'class':'dealDate'}).text.strip()})
        info_dict.update({u'签约单价':cj.find('div',{'class':'unitPrice'}).text.strip()})
        info_dict.update({u'签约总价':cj.find('div',{'class':'totalPrice'}).text.strip()})

        if(cj.find('div',{'class':'dealHouseInfo'})):
            content=cj.find('div',{'class':'dealHouseInfo'}).text.split()
            if content:
                for c in content:
                    if c.find(u'满')!=-1:
...
def xiaoqu_chengjiao_spider(db_cj,xq_name=u"冠庭园"):
    ...
    爬取小区成交记录
    """
    url=u"http://bj.lianjia.com/chengjiao/rs"+urllib.parse.quote(xq_name)+"/"
    try:
        req = urllib.request.Request(url,headers=hds[random.randint(0,len(hds)-1)])
        source_code = urllib.request.urlopen(req,timeout=10).read()
        plain_text=source_code.decode('utf-8')#,errors='ignore')
    ...
    if content:
        ...
        # a = content.get('page-data')
        # print(html.unescape(a))
        # d = json.loads(a)
        # print(d)
        # d='d={"totalPage":26,"curPage":1}'
        #exec(d)
        #total_pages=d['totalPage']
